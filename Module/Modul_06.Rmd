---
title: "Modul 06: Nett oder schön, warum nicht und?"
output: 
  learnr::tutorial:
    language: 
...
- das wir Zusammenhänge sehen, die wir ungewollt erzeugen.
...
Wieder stark vereinfacht: Fürs Daten sei Nettigkeit und Aussehen wichtig. 

Würden Sie jemanden Daten, der weder nett ist noch gut aussieht? Vielleicht, aber eigentlich eher nicht.
...
Sie Daten also jemanden, der nett ist *oder* gut aussieht.

Dieses angenommene Modell lässt sich als kausales Diagramm wie folg darstellen:
...
\color{green}{Y} &=& U_{\color{blue}{Y}}\\
\color{violet}{Z} &=& f_{\color{violet}{Z}}(\color{green}{X}, \color{blue}{Y}, U_{\color{violet}{Z}})
\end{eqnarray*}


```{r abhaengigkleit, echo=FALSE}
question("Hängt Aussehen ($Y$) von Nettigkeit ($X$) ab?",
         answer("Ja", message = "Nach Konstruktion sind $X$ und $Y$ unabhängig voneinander. Kein kausaler Pfad führt von $X$ zu $Y$ - oder umgekehrt."),
         answer("Nein", correct = TRUE, message = "Nach Konstruktion sind $X$ und $Y$ unabhängig voneinander. Kein kausaler Pfad führt von $X$ zu $Y$ - oder umgekehrt."),
...
In Fällen wie diesen: $$\color{green}{X} \rightarrow \color{violet}{Z} \leftarrow \color{blue}{Y}$$ wird die Variable in der Mitte - hier $\color{violet}{Z}$ - **Collider** genannt. $\color{violet}{Z}$ ist eine  Wirkung von $\color{green}{X}$ und $\color{blue}{Y}$.
...
question("Angenommen Sie haben jemanden gedatet ($Z$), der nicht besonders nett ($X$) ist. Wissen Sie dann etwas über das Aussehen ($Y$)?",
         answer("Ja", correct = TRUE, message = "Wenn es kein Glück oder Pech ($U_Z$) war, dann ist nicht-nette Person eher hübsch. Es gibt ja einen Grund dafür, dass Sie sie daten. Wenn es nicht die Nettigkeit ist, dann das Aussehen."),
         answer("Nein", message = "Wenn es kein Glück oder Pech ($U_Z$) war, dann ist nicht-nette Person eher hübsch. Es gibt ja einen Grund dafür, dass Sie sie daten. Wenn es nicht die Nettigkeit ist, dann das Aussehen."),
		correct = random_praise(),
		incorrect = random_encouragement()
         )
```
...
$\mathcal{N}(0,\,1)$ steht für eine eine Standardnormalverteilung, $\mathcal{B}(0.05)$ für eine Bernoulliverteilung mit $\pi=0.05$. $\vee$ ist das logische *oder* (`|` in `R`). 

Folgender `R` Code simuliert diesen datengenerierenden Prozess:

```{r sim, eval=FALSE}
library(mosaic) # Paket laden
set.seed(1996) # Zufallszahlengenerator setzen
...
Ok, sowohl die mathematische Darstellung als auch der `R` Code sind anspruchsvoll. Worauf es in diesem Modul viel mehr ankommt sind die Daten, die dabei rauskommen:
...
```{r corb, echo=FALSE}
question("Für die, Sie gedatet haben (`z = Ja`): Sehen Sie einen Zusammenhang zwischen Nettigkeit `x` und Aussehen `x`?", 
         answer("Ja", correct = TRUE, message = "Die Regressionsgerade geht von links oben nach rechts unten. Dies zeigt eine negative Korrelation an. Tendenziell sind die Dates, die hübsch sind nicht besonders nett - und umgekehrt."),
         answer("Nein", message = "Die Regressionsgerade geht von links oben nach rechts unten. Dies zeigt eine negative Korrelation an. Tendenziell sind die Dates, die hübsch sind nicht besonders nett - und umgekehrt."),
		correct = random_praise(),
		incorrect = random_encouragement()
         )
```
...
Wenn wir <violet>Date</violet> ($\color{violet}{Z}$) berücksichtigen, sehen wir einen Zusammenhang zwischen <blue>Aussehen</blue> ($\color{blue}{Y}$) auf <green>Nettigkeit</green> ($\color{green}{X}$), obwohl nach Konstruktion gar keiner da ist.
Das sehen wir auch, wenn wir uns den Zusammenhang zwischen $\color{green}{X}$ und $\color{blue}{Y}$ ohne Berücksichtigung von $\color{violet}{Z}$ angucken:
...
Und diese liefert ein verzerrtes Ergebnis.

## Zusammenfassung

:::{.box}
Um den (totalen) kausalen Effekt von $X$ auf $Y$ in einer umgedrehten Gabel $$X \rightarrow Z \leftarrow Y$$ zu bestimmen, sollte ein Collider $Z$ nicht berücksichtigt werden. (Und auch keine Nachfahren von $Z$.)
Wird $Z$ berücksichtigt wird ein scheinbarer Zusammenhang zwischen $X$ und $Y$ erzeugt, d.h. fließt in die Analyse mit ein.
::: 
