...
Wir hoffen dass dieser Kurs und seine Beispiele Ihnen helfen kann systematische *Verzerrung* zu adressieren.

*Rauschen* entsteht durch (zufällige) Stichprobenvariation.
...
In der Realität sieht es natürlich viel komplexer aus &ndash; Graphen können hunderte Variablen beinhalten, oft auch unbeobachtbare.
...
Schlußfolgerungen sind fast immer mit gewissen Unsicherheiten, z. B. aufgrund von Rauschen, behaftet und Modelle sind immer eine vereinfachte Darstellung der Realität.

Selbst wenn wir nicht den kompletten kausalen Graphen kennen, hilft uns das Wissen über die fundamentalen kausalen Strukturen, kausale Schlußfolgerungen kritisch zu hinterfragen und mögliche Verzerrungen zu erkennen.
...
```{r kaffee, echo=FALSE}
question("Auf welche fundamentale kausale Struktur zwischen Kaffeekonsum, Übelkeit und Fehlgeburt deutet diese Erklärung hin?",
  answer("Eine Kette"),
...
question("Welche Rolle spielt \"Bis 5 überleben\" im Teilgraphen mit \"Intrinsische Robustheit\" und \"Art der Gesellschaft\"",
  answer("Mediator"),
  answer("Confounder"),
  answer("Collider", correct = TRUE, message = "Korrekte &ndash; die beiden Pfeile zeigen beide auf die \"Bis 5 überleben\""),
         allow_retry = TRUE,
         correct = random_praise(),
         incorrect = random_encouragement())
```
...
Hoffentlich konnten wir Ihnen einige Werkzeuge an die Hand geben, um kausale Schlüsse kritisch zu hinterfragen.
...
Jonas Peters (den kennen Sie aus einem der Videos) hat zusammen mit Dominik Janzing und Bernhard Schölkopf auch ein Buch geschrieben: [Elements of causal inference: foundations and learning algorithms](https://mitpress.mit.edu/books/elements-causal-inference).
Das Thema kausale Inferenz wird im Buch [Statistical Rethinking](https://xcelab.net/rm/statistical-rethinking/) im 6. Kapitel der zweiten Auflage Richard McElreath behandelt &ndash; ihn kennen Sie auch aus den Videos.
...
Zur Analyse können dabei häufig sogenannte **Instrumentvariablen** oder **Regressions-Diskontinuitäts-Analyse** angewendet werden. Oder es wird die **Differenz-von-Differenzen** betrachtet. 

Für Arbeiten in diesem Bereich wurde übrigens im Jahre 2021 der Alfred-Nobel-Gedächtnispreis für Wirtschaftswissenschaften an David Card, Joshua Angrist und Guido Imbens verliehen! Herzlichen Glückwunsch! Wie diese Methoden helfen wichtige Fragen zu beantworten hat die Schwedischen Akademie der Wissenschaften [hier](https://www.nobelprize.org/uploads/2021/10/popular-economicsciencesprize2021-3.pdf) beschrieben.
