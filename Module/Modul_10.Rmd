...
- daraus die Konsquenz für die kausale Schlussfolgerung zu ziehen;
- eine Simulation für den Gender-Pay-Gap in R.
...
Kann das sein? Mensch denkt doch immer eher, das Frauen im Job (und beim Gehalt) *benachteiligt* werden.
...
Dies haben Sie in diesem Kurs gelernt.
...
question("Gibt es einen Pfeil zwischen *Fähigkeit* und *Geschlecht*?",
  ...
  answer("Nein.", correct = TRUE, message = "Wir nehmen an, dass es keinen Zusammenhang zwischen *Fähigkeit* und *Geschlecht* gibt, daher gibt es auch keinen Pfeil."),
  allow_retry = TRUE,
...
question("Gibt es einen Pfeil zwischen *Fähigkeit* und *Vorstand*?",
  answer("Ja, in der Form *Fähigkeit* $\\rightarrow$ *Vorstand*." , correct = TRUE, message = "Wir nehmen an, dass die Vorstandstätigkeit von der Fähigkeit abhängt &ndash; und nicht umgekehrt."),
    answer("Ja, in der Form *Vorstandt* $\\rightarrow$ *Fähigkeit*."),
  answer("Nein."),
  allow_retry = TRUE,
  correct = random_praise(),
  incorrect = random_encouragement()
...
*Hinweis*: Für die Diagramme innerhalb dieses Kurses wurde das Paket `ggdac` ([https://ggdag.malco.io/](https://ggdag.malco.io/)) verwendet.

Zeichnen Sie dort - oder auf einem Blatt Papier - das Diagramm, dass die Annahmen
...
*Klicken Sie erst danach zur Kontrolle auf `Nächstes Thema`*
...
3. Ein Mann kommt ab einer `faehigkeit` größer als 115 in den Vorstand, eine Frau erst ab 130. Frauen müssen also fähiger sein als Männer um in den Vorstand zu kommmen &ndash; so hier die Annahme.
...
<red>&#9888;</red> Auch diese Annahmen sind stark vereinfacht!
...
Wie konstruiert: Es gibt keine nennenswerten Unterschiede (hier: im arithmetischen Mittelwert, engl. mean, Funktion `mean()`) in der Fähigkeit zwischen den Geschlechtern:
...
Wie konstruiert: Männer verdienen im Durchschnitt mit `r round(mean(gehalt ~ geschlecht, data = GPG))[1]` mehr als Frauen, deren Durchschnittsgehalt hier bei `r round(mean(gehalt ~ geschlecht, data = GPG))[2]` liegt.

## Analyse 2

Was passiert, wenn wir uns auf die Vorstandsmitglieder beschränken, d.h., wir wählen nur die Beobachtungen aus (`filter()`), die Bedingung erfüllen, dass sie im Vorstand sind (`vorstand == "j"`)):
...
Obwohl Männer insgesamt nach Konstruktion mehr verdienen, verdienen Frauen im Vorstand mehr.
...
Und das Gehalt hängt ja, neben dem Geschlecht und der Tätigkeit im Vorstand auch von der Fähigkeit ab &ndash; so die Annahme.

## Verzerrung

Wie in Modul 6 besprochen: Adjustierung eines Colliders erzeugt eine Verzerrung, es wird ein Scheinzusammenhang, der den Effekt des Geschlechts auf das Gehalt verzerrt. Mit [DAGitty](http://dagitty.net/) kann dies schön gezeigt werden:

![](images/GPG-2.png){width="65%"}
