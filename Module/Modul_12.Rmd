...
ranksep = 0.5;
 graph []
 node [shape = plaintext fontcolor = blue fontname = Poppins fontsize = 6]
   A [label = 'Kaffeekonsum']
   B [label = 'Fehlgeburt']
   C [label = 'Übelkeit']
   U
 edge [minlen = 2]
...
rankdir = LR;
 ...
 node [shape = plaintext fontcolor = blue fontname = Poppins fontsize = 11]
   ...
   B [label = 'Art der Gesellschaft']
   C [label = 'Bis 5 Überleben']
   D [label = 'Bis 70 Überleben']
 edge [minlen = 2]
   {A; B} -> {C; D} [penwidth = .5]
        C -> D      [penwidth = .5]
}
...
question("Auf welche fundamentale kausale Struktur zwischen Kaffeekonsum, Übelkeit und Fehlgeburt deutet diese Erklärung hin?",
  ...
  answer("Eine Gabel", correct = TRUE, message = "Übelkeit beeinflußt den Kaffeekonsum, Übelkeit korreliert mit einem niedrigeren Risiko für eine Fehlgeburt.
         Damit könnte Übelkeit ein Confounder sein, oder zumindest auf einem konfundierendem Pfad liegen bei dem eine unbeobachtete Variable sowohl Übelkeit als auch das Fehlgeburtsrisiko beeinflußt."),
...
<center>
```{r p1, echo=FALSE, out.widht="40%"}
p1
```
</center>

Studien, die für Übelkeit als potenziellen Confounder adjustieren, finden tendenziell schwächere Zusammenhänge.

Die Idee, dass Übelkeit ein Confounder ist, wird auch dadurch gestützt, dass Studien insgesamt weniger konsistente Zusammenhänge zwischen Fehlgeburten und dem Konsum anderer koffeinhaltiger Getränke (z. B. Tee oder Cola) finden.
...
Diese Beobachtungen werden manchmal aufgegriffen, um Argumente zu verschiedenen Lebensstilen zu haben.
...
Das Thema kausale Inferenz wird im Buch [Statistical Rethinking](https://xcelab.net/rm/statistical-rethinking/) im 6. Kapitel der zweiten Auflage von Richard McElreath behandelt &ndash; ihn kennen Sie auch aus den Videos.
