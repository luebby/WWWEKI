...
Hier können graphische Verfahren, Kennzahlen, je nach Fragestellung und Datenbasis auch Verfahren der Inferenzstatistik (p-Werte, Konfidenzintervalle, Bayes-Statistik) verwendet werden, um z.B. zu untersuchen wie hoch der Anteil der Raucherinnen unter den Frauen war, leider einen Schlaganfall erlitten haben. Aber auch Verfahren wie Clusteranalyse oder Hauptkomponentenanalyse können hierfür verwendet werden. 
...
question("Ist das Ergebnis einer Datenanalyse immer geeignet das zu untersuchende Phänomen zu beschreiben?",
         answer("Ja", message = "Daten sind i.d.R. nur ein Teil der komplexen und dynamischen Realität. Z.B. Messfehler, nicht-repräsentative Stichproben, fehlende Werte können zu einem verzerrten Ergebnis führen so dass die Datenbeschreibung in die Irre führt. Auch für diese Aufgabenstellung muss die Datenentstehung immer kritisch hinterfragt werden."),
...
In den Anwendungen Beschreibung und Vorhersage wurden die Daten (zunächst) *nur* beobachtet. Es erfolgte zunächst keine Handlung - auch wenn wie in der Übung (wie häufig bei Algorithmischen Entscheidungssystemen) aus der Vorhersage häufig eine Handlung folgt. 
...
question("Kann es einen Unterschied in der Verteilung von $Y$ geben, je nach dem ob $X$ beobachtet wird ($X=x$) oder auf Grund einer Handlung $X$ gesetzt wird ($do(X=x)$)?",
         answer("Ja", correct = TRUE, message = "Es kann sein, dass eine weitere Variable ($Z$) Ursache für $X$ und $Y$ ist. Ich sehe einen Zusammenhang zwischen $X$ udnd $Y$, aber dieser liegt an $Z$. Wenn ich $X$ ändere, ändert sich $Y$ nicht. Wenn ich $Z$ ändere, beobachte ich einen anderen Wert von $X$ und $Y$."),
         answer("Nein", message = "Es kann sein, dass eine weitere Variable ($Z$) Ursache für $X$ und $Y$ ist. Ich sehe einen Zusammenhang zwischen $X$ udnd $Y$, aber dieser liegt an $Z$. Wenn ich $X$ ändere, ändert sich $Y$ nicht. Wenn ich $Z$ ändere, beobachte ich einen anderen Wert von $X$ und $Y$."),
		correct = "Klasse! Sie können sicht gut an das 2. Modul dieses Kurses erinnern",
		incorrect = "Leider falsch. Vielleicht gucken Sie sich noch einmal das 2. Modul (Ein Pfeil zeigt die Richtung) an."
         )
```
...
question("Im Rahmen des Kundenbeziehungsmangements ist Kundenabwanderung ein wichtiges Thema. Was ist das Ziel der Datenanwendung: *Welche Kund:innen haben eine hohe Abwanderungsrate?*",
