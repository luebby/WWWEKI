---
title: "Modul 11: Schadet Rauchen bei Heranwachsenden?"
output: 
  learnr::tutorial:
    language: 
...
- die Bestimmung eines kausalen Effekts mit Hilfe einer linearen Regression in `R` an einem realen Beispiel.
...
question("Ist hier ein randomisiertes Experiment ethisch vertretbar?",
  answer("Ja.", message = "Es können negative Folgen bei den Teilnehmer\\*innen, die im Experiment den Raucher\\*innen zugeordnet werden, befürchtet werden."),
  answer("Nein.", correct = TRUE, message = "Negative Konsequenzen sind für die Raucher\\*innen zu erwarten."),
  allow_retry = TRUE,
...
Bedingt/ gefiltert nach `RaucherIn` ergibt sich folgende Abbildung:
...
Wenn wir den Code erweitern, so dass Sie die Punkte gemäß der Variable `RaucherIn` farbig markiert sind, sehen wir: Bei Heranwachsenden sind die Raucher:innen älter als die Nichtraucher:innen &ndash; und damit auch größer.
...
Weil die nicht-kausalen Pfade durch die *Hintertür* (engl. backdoor, Gabel) offen sind:
...
question("Sollte über die Variable `Groesse` adjustiert werden?",
  answer("Ja."),
  answer("Nein.", correct = TRUE, message = "Die Variable `Groesse` liegt auf dem kausalen Pfad (*Kette*) zwischen `RaucherIn` und `Lungenvolumen` (Mediator)."),
         allow_retry = TRUE,
         correct = random_praise(),
         incorrect = random_encouragement())
```
...
question("Hat in dem Modell Rauchen einen negativen Effekt auf das Lungenvolumen?",
  answer("Ja.", correct = TRUE, message = "Der geschätzte Koeffizient ist mit $-0.153974$ negativ (`Estimate` für die Variable `RaucherInja `)."),
  answer("Nein."),
  allow_retry = TRUE,
  correct = random_praise(),
  incorrect = random_encouragement())
...
*Anmerkung*: Die Annahme eines linearen Modells ist aufgrund des nicht-linearen Zusammenhangs zwischen Alter und Größe sowie Größe und Lungenvolumen (siehe Abbildungen im Abschnitt *Das Alter ...*) eher nur eine Näherung.
