...
- dass wir manchmal ungewollt selber Zusammenhänge schaffen.
...
Wieder stark vereinfacht: Nehmen wir an, dass beim Dating Nettigkeit und Aussehen eine Rolle spielen.
...
message <- "Wenn es kein Glück oder Pech ($U_Z$) war, dann sieht die nicht-nette Person eher gut aus. Es gibt ja einen Grund dafür, dass Sie sie daten. Wenn es nicht die Nettigkeit ist, dann das Aussehen."
...
Bei einer **Gabel** gibt es einen nicht-kausalen Pfad von $\color{green}{X}$ nach $\color{blue}{Y}$:
$$\color{green}{X} \leftarrow \color{violet}{Z} \rightarrow \color{blue}{Y}$$
Zwar *hört*  $\color{blue}{Y}$ auf $\color{violet}{Z}$, $\color{violet}{Z}$ aber nicht auf $\color{green}{X}$, sondern umgekehrt, $\color{green}{X}$ folgt aus $\color{violet}{Z}$. 
...
<green>Eis</green> und <blue>Sonnencreme</blue> sind (*unbedingt*, *marginal*) nicht unabhängig: Wenn ich weiß, dass viel <green>Eis</green> verkauft wurde, kann ich davon ausgehen, dass auch viel <blue>Sonnencreme</blue> verkauft wurde. Vermutlich führte <violet>Sonnenschein</violet> zu hohem <green>Eisverkauf</green> und so auch zu hohem <blue>Sonnencremeverkauf</blue>. 
Über die Information <green>Eis</green> kann man über <violet>Sonnenschein</violet> etwas über <blue>Sonnencreme</blue> lernen.
Wenn ich weiß, dass <violet>Sonnenschein</violet> ist, enhält die Information, dass viel <green>Eis</green> verkauft wurde, keine zusätzliche Information über <blue>Sonnencreme</blue>. *Bedingt* <violet>Sonnenschein</violet>, d. h., ich weiß es scheint die Sonne, sind <green>Eis</green> und <blue>Sonnencreme</blue> unabhängig.
...
$\color{violet}{Z}$ *hört* aber auf $\color{green}{X}$ und $\color{blue}{Y}$. 
...
Wenn ich weiß, dass ich <violet>ausschlafen</violet> kann, weiß ich, dass entweder <green>Wochenende</green> oder <blue>Urlaub</blue> (oder beides) ist. Bei <violet>Ausschlafen</violet> lerne ich aus der Information <blue>kein Urlaub</blue>, dass <green>Wochenende</green> ist.
Einen Grund muss mein Ausschlafen ja haben.
*Bedingt*, gegeben, <violet>Ausschlafen</violet> sind <green>Wochenende</green> und <blue>Urlaub</blue> nicht mehr unabhängig.
...
$\mathcal{N}(0,\,1)$ steht für eine eine Standardnormalverteilung, $\mathcal{B}(0.05)$ für eine Bernoulliverteilung mit $\pi=0.05$. 
...
Wenn wir <violet>Date</violet> ($\color{violet}{Z}$) berücksichtigen, sehen wir einen Zusammenhang zwischen <blue>Aussehen</blue> ($\color{blue}{Y}$) und <green>Nettigkeit</green> ($\color{green}{X}$), obwohl nach Konstruktion gar keiner da ist.
...
Die Variable $Z$ sollte beispielsweise nicht als erklärende Variable in ein lineares Modell aufgenommen werden.
::: 
