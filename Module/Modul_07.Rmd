...
- das aus einer Beobachtung nicht immer eine Handlung abgeleitet werden kann.
...
message <- "Die anhand der Stichprobe im linearen Modell geschätzte Steigung liegt bei $\\hat{\\beta}_1= 22573$, der geschätzte Achsenabschnitt, also der Mittelwert des Preises bei $x=0$ Räumen, $\\hat{\\beta}_0=53016$."
question("Im linearen Modell der Stichprobe: Um wieviel erhöht sich der Preis pro Raum?",
         answer("53016", message = message),
...
```{r graph1, echo=FALSE}
message <- "Neben den direkten Effekt über $Z \\rightarrow Y$ gibt es auch den indirekten $Z \\rightarrow X \\rightarrow Y$. Hierbei ist die Anzahl Räume ein Mediator."
question("Welcher Teilgraph liegt vor, wenn die Beziehung zwischen Fläche und Preis im Zusammenhang mit der Anzahl Räume analysiert wird?",
         ...
         answer("Gabel", message = message),
         answer("Umgedrehte-Gabel", message = message),
		correct = random_praise(),
		incorrect = random_encouragement()
         )
```
...
```{r graph2, echo=FALSE}
message <- "Neben den direkten Effekt über $X \\rightarrow Y$ gibt es auch den nicht-kausalen $X \\leftarrow Z \\rightarrow Y$. Hier ist die Fläche ein Confounder."
