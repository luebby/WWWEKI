...
Stellen Sie sich folgende Situation vor: Am Ende Ihrer Ausbildungszeit wird Ihnen die Teilnahme an einer freiwillige Schulung zum Thema Gehaltsverhandlungen angeboten.
...
Wie würden Sie für sich diesen **individuellen kausalen Effekt** der Teilnahme an der Schulung auf Ihr Gehalt definieren?
...
Der individuelle kausale Effekt $\color{orange}{\Delta}_i$ lässt sich dann in `R` innerhalb der Datentabelle `Schulung` berechnen als `gehalt1 - gehalt0`:

***

*Anmerkung:* `R` ist eine freie Software für die Datenanalyse. Sie kann von der Seite [https://cran.r-project.org/](https://cran.r-project.org/) für gängige Betriebssysteme heruntergeladen werden. Ergänzend ist die Nutzung des `RStudio Desktop` empfehlenswert ([https://www.rstudio.com/products/rstudio/download/](https://www.rstudio.com/products/rstudio/download/)). Im Rahmen dieser Module für außerdem u.a. das Paket `mosaic` ([https://cran.r-project.org/package=mosaic)](https://cran.r-project.org/package=mosaic)) verwendet.
...
question("Können Sie diese individuellen kausalen Effekte in der Realität beobachten?",
         answer("Ja", message = "Für jede Beobachtung $i$ liegt nur ein Gehaltswert vor: der mit Schulung oder der ohne Schulung, aber niemals beide gleichzeitig, da Personen entweder teilnehmen oder nicht. $\\Delta_i$ kann daher nicht beobachtet werden."),
...
- Wenn $i$ nicht an der Schulung teilnimmt, liegt uns das Gehalt ohne Schulung vor, $\color{blue}{Y}^{\color{green}{X=0}}_i$, und nicht $\color{blue}{Y}^{\color{green}{X=1}}_i$
...
question("Person $i$ nimmt an der Schulung teil. Was ist dann das Counterfactual?",
         answer("$\\color{blue}{Y}^{\\color{green}{X=0}}_i$", correct = TRUE, message = "Teilnahme an der Schulung bedeutet $X=1$."),
         answer("$\\color{blue}{Y}^{\\color{green}{X=1}}_i$", message = "Teilnahme an der Schulung bedeutet $X=1$."),
		correct = random_praise(),
		incorrect = random_encouragement()
         )
```
...
$$\bar{\color{orange}{\Delta}}=\frac{\sum_{i=1}^n \color{orange}{\Delta}_i}{n}=\frac{\sum_{i=1}^n\color{blue}{Y}^{\color{green}{X=1}}_i - \sum_{i=1}^n\color{blue}{Y}^{\color{green}{X=0}}_i}{n}=\overline{\color{blue}{Y}}^{\color{green}{X=1}}-\overline{\color{blue}{Y}}^{\color{green}{X=0}}.$$
...
*Anmerkung:* In `mosaic` ist der `R` Syntax vereinheitlicht:
...
Sollte es, wie im vorliegenden Fall, keine erklärende Variable geben, kann auch kurz geschrieben werden:
...
In der Praxis kennen wir zwar nicht beide Potential Outcomes, aber wir wissen, wer an der Schulung teilgenommen hat und wer nicht. Daraus ergibt sich, welches der beiden Potential Outcomes beobachtet ist und welches uns fehlt (das Counterfactual). Somit können wir unsere fiktive Gehaltstabelle aktualisieren, damit sie eine realistische Datensituation widerspiegelt.

Fehlende Werte werden in `R` als `NA` (engl.: not available, nicht verfügbar) dargestellt.
...
Was wir in unseren Daten also sehen ist das tatsächlich beobachtete `gehalt` und, ob am Seminar teilgenommen wurde (`teilnahme`).
...
```{r bias, echo=FALSE}
question("Die Teilnahme an der Schulung war freiwillig. Können Sie davon ausgehen, dass, unabhängig von der Schulung, Teilnehmende und Nichtteilehmende eine ähnliche Gehaltsentwicklung erlebt hätten?",
         answer("Ja", message = "Teilnehmende und Nichtteilnehmende könnten sich in vielerlei Hinsicht unterscheiden. Um nur ein Beispiel zu nennen: Personen, denen ihr Gehalt besonders wichtig ist, werden eher an der Schulung teilnehmen. Diese Personen werden aber vielleicht auch ohne Schulung anders in Gehaltsverhandlungen auftreten."),
...
Dieser Wert weicht aber substanziell vom wahren durchschnittlichen kausalen Effekt ab, den wir zuvor mithilfe der Potential Outcomes berechnet hatten: $\bar{\color{orange}{\Delta}} =`r round(mean(~ effekt, data = Schulung),2)`$ auf.
Bei dieser Abweichung handelt es sich um eine systamtische Verzerrung, die auch als **Bias** bezeichnet wird.


Der naive Vergleich der Mittelwerte überschätzt hier den wahren durchschnittlichen Effekt. In unserem fiktiven Beispiel liegt das daran, dass Personen mit einem höherem Gehaltsinteresse eher an der Schulung teilgenommen haben als die mit einem niedrigeren Interesse. Erstere hätten aber, aufgrund Ihres Gehaltsinteresses, so oder so, also unabhängig von der Schulung, ein höheres Gehalt erwartet. Natürlich sind hier auch andere inhaltliche Erklärungen denkbar.

Kontrollieren Sie diese Aussage, indem Sie den Code so abändern, dass Sie den Mittelwert der Potential Outcomes, `gehalt0` und `gehalt1`, berechnen:

*Tipp*: Einen Lösungshinweis erhalten Sie über `Hinweise`. Anschließend `Nächster Tipp` und Sie sehen die Lösung.
...
Das fundamentale Problem der kausalen Inferenz ist universell. In vielen Situationen kann es dazu führen, dass die Daten uns in die Irre führe: Vielleicht vermuten wir einen kausalen Effekte wo es nicht wirklich einen gibt, oder wir erkennen real existierende Effekte nicht. 
...
In den folgenden Modulen werden werden Sie verschiedene datengenerierende Mechanismen auf abstrakter Ebene kennenlernen und erfahren, wie sie kausale Schlüsse beeinflussen.
