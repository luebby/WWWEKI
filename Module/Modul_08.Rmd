...
Was Sie sehen: <green>Kaufer:innen von Bio-Lebensmitteln</green> haben viel öfter eine hohe <blue>Lebenserwartung</blue> als diejenigen, die keine Bio-Lebensmittel kaufen &ndash; in dieser fiktiven Studie.
...
Nehmen wir an, bei der Datenerhebung gibt es einen Zusammenhang zwischen dem Geschlecht und der Stichprobe. Z. B. hier, dass Frauen eine höhrere Wahrscheinlichkeit haben Teil der Stichprobe zu sein, wie es das folgende kausale Diagramm verdeutlicht:
...
Wenn wir das Ergebnis verwenden würden, um z. B. vorherzusagen ob ein Kunde Bio kauft oder nicht, wäre eine Kaufswahrscheinlichkeit von  $Pr(\color{purple}{\text{Bio}}) = `r pdgs`$ zu hoch.
...
Das Problem ist, dass unsere Stichprobe von einer Variable abhängt, dem <violet>Geschlecht</violet> von dem wiederum die untersuchte Variable <green>Kauf Bio-Lebendmittel</green> abhängt.
Dadurch dass hier Frauen eine höhrere Wahrscheibnlichkeit haben Teil der Stichprobe zu sein und häufiger Bio-Lebensmittel kaufen überschätze ich in Folge mit meiner Stichprobe den Anteil der Käufer:innen von Bio-Lebensmittel.
...
Und je größer die Stichrobe desto weniger schwankt der Anteil bei unterschiedlichen Stichproben.
...
```{r out.width="80%", echo = FALSE, fig.asp = .7, fig.align="center"}
gf_bar(~ lebenserwartung | kauf , data = d2, 
       fill = ~ lebenserwartung ) + 
  scale_fill_manual(values = c("#A9BCF5", "#0B2161")) +
  labs(title = "Beobachtungsstudie") +
...
```{r out.width="65%", echo = FALSE, fig.asp = .7, fig.align="center"}
gf_bar(~ kauf | geschlecht , data = d2, 
       fill = ~ kauf ) + 
...
Frauen kaufen häufiger <purple>Bio</purple> als Männer - und haben öfter eine hohe Lebenserwartung.
